...
public class Text : Message
{
    ...
    public Text(ITelegramBotClient client, Update update, CancellationToken ctocken)
    {
        _client = client;
        _update = update;
        _ctoken = ctocken;
    ...
    public override async Task<GenericResultExtending<TBMessage>?> Send(string text, bool reply = true, bool markdownMode = false)
    {
        try
        {
            ...
            if (chatId is null) 
                return (GenericResultExtending<TBMessage>)ErrorManager.GetError(ErrorEnum.NoChatId)!;
